...
function buscarProductos() {
    const opcionBusqueda = prompt(`Elija la opción deseada:\n1. Buscar por marca\n2. Buscar por genero\n3. Buscar por disciplina\n4. Buscar por modelo\n5. Volver`);
    ...
    if (opcionBusqueda === "1") {
        const productosEncontrados = buscarPorMarca();
    ...
    const buscarPorMarca = () => {
        let marcasDisponibles = "Marcas disponibles:\n";
        productos.forEach((producto, index) => {
            marcasDisponibles += `${index + 1}. ${producto.marca}\n`;
        });
        
        marcasDisponibles += "Ingrese el número de la marca que desea buscar:";
        
        const marcaIndex = Number(prompt(marcasDisponibles));
    
        if (marcaIndex >= 1 && marcaIndex <= productos.length) {
            const marcaElegida = productos[marcaIndex - 1].marca;
            return productos.filter((producto) => producto.marca === marcaElegida);
        } else {
